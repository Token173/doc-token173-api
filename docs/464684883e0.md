...
paths:
  /v1/chat/completions:
    post:
      ...
      deprecated: false
      description: >
        本接口为兼容OpenAI ChatGPT 的聊天非流式接口。

        所有支持对话的模型都可以使用该接口。

        图片、视频、音频相关接口可以从左侧列表中找到对应接口。
        ...
        2、将 API令牌 设置在Header Authorization 的 YOUR_API_KEY 参数中。

        3、将 模型名称 填写在Body model 参数中。

        4、将 提示词 填写在Body messages[0].content 中。

        5、点击 发送 请求进行调试。

        6、调试成功可以在当前文档最下面查看各种语言的“请求示例代码”，参考示例代码进行接口对接。
      ...
      parameters:
        ...
        - name: Authorization
          in: header
          description: YOUR_API_KEY设置成你的API令牌
          required: false
            ...
            schema:
              ...
              properties:
                model:
                  type: string
                  description: |+
                    要使用的模型名称。有关哪些模型可与聊天 API 一起使用的详细信息,请参阅模型端点兼容性表。
      ...
      x-run-in-apifox: https://app.apifox.com/web/project/8343734/apis/api-464684883-run
components:
  schemas: {}
  securitySchemes:
    bearer:
